...
Samsung Xpress M2020은 **삼성 공식 ULD(Unified Linux Driver)의 `rastertospl` 필터 + `Samsung_M2020_Series.ppd`로만 정상 인쇄됩니다.**

핵심 제약 두 가지와 해법:

1. **splix는 쓰면 안 됩니다.** 오픈소스 SpliX(ML-2160 PPD, QPDL v3)로 인쇄하면 M2020 **펌웨어가 데이터를 받다가 크래시**합니다 — job은 "완료"로 뜨지만 종이는 안 나오고, `dmesg`에 job 전송 완료 시각마다 USB disconnect → 8초 뒤 재연결이 찍힙니다 (2026-07-14 실측). 반면 ULD가 만든 SPL 데이터는 raw로 보내도 정상 인쇄됨을 확인했습니다.
2. **ULD는 x86 바이너리 전용**이라 aarch64에서 직접 실행이 안 됩니다. 그래서 이 설정은:
   - `flake.nix`에서 **x86_64용 ULD 패키지**(`pkgsX86.samsung-unified-linux-driver_1_00_36`, unfree)를 클로저에 넣고,
   - `boot.binfmt.emulatedSystems = [ "x86_64-linux" ]`로 Pi에서 **qemu 유저모드 에뮬레이션**으로 `rastertospl`을 실행합니다.
   - 렌더링(ghostscript)은 네이티브 aarch64로 돌고, raster→SPL 변환만 에뮬레이션되므로 페이지당 수십 초 수준의 지연만 있습니다.

참고: 과거 nixpkgs splix의 `QPDLVersion` 누락 이슈([NixOS/nixpkgs#36876](https://github.com/NixOS/nixpkgs/issues/36876))는 현재 SVN 스냅샷(r315)에서 해결됐지만, 그것과 무관하게 M2020은 QPDL 자체를 못 받습니다.
...
- **job은 완료되는데 종이가 안 나오고, `dmesg`에 전송 완료 시각마다 `USB disconnect`가 찍히면** 프린터가 인쇄 데이터를 못 받아들여 크래시하는 것입니다 (잘못된 드라이버/PPD). 큐의 PPD를 확인하세요: `grep NickName /etc/cups/ppd/Samsung_M2020.ppd`가 **"Samsung M2020 Series"**(ULD)여야 하며, "Samsung ML-2160"(splix)이면 잘못된 것입니다.
- 인쇄가 유난히 느리면 정상입니다 — `rastertospl`이 qemu 에뮬레이션으로 돌기 때문에 페이지당 수십 초 걸릴 수 있습니다.
